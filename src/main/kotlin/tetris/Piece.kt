...
data class Piece(val type: Tetromino, val blocks: List<Block>)
...
fun Canvas.drawPiece(p: Piece) {
	p.blocks.forEach { drawBlock(it) }
}

fun Piece.move(dx: Int, dy: Int, f: List<Block>): Piece? {
	val bs: List<Block> = blocks.mapNotNull { b -> b.move(dx, dy, f) }
	return if (bs.size == 4) Piece(type, bs) else null
}

fun Piece.toFix(f: List<Block>) = blocks.any { it.toFix(f) }

fun Piece.rotate(f: List<Block>): Piece? {
	if (type == Tetromino.O) return null
	val bs = blocks.mapNotNull { b -> b.rotate(blocks[0], f) }
	return if (bs.size == 4) Piece(type, bs) else null
}
