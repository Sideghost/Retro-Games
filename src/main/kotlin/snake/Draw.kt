package snake// Import of Canvas and the hex of the colors
...
private const val MENU_TEXT_X_ORIGIN = MENU_Y_ORIGIN - CORRECTION   // X origin of text in menu.
private const val MENU_TEXT_Y_ORIGIN = 170                          // Y origin of text in menu.
...
/**
 * Draws the whole game (snake.Snake, Walls, Apple, snake.Status).
...
fun Canvas.drawGame(game: Game) {
    ...
    game.wall.forEach { drawBrick(it, "snakeResources\\bricks.png") }
    drawApple(game.apple, "snakeResources\\snake.png")
    drawStatus(game)
    drawHackedApple(game.hacking.hackedApple, if (game.hacking.level == 2)"snakeResources\\appleGolden.png" else "snakeResources\\applePoison.png")
    if (game.hacking.menu) drawMenu()
}


/**
 * Function that draws the snake.Snake in the canvas using a Sprite.
 * @receiver where it will be drawn.
...
/**
 * Function that draws the Head of the snake.Snake in the canvas using a Sprite.
 * @receiver where it will be drawn.
 * @param snake object with movement in the game.
 * @param pngFile input file that has the drawing of the snake.
 */
...
/**
 * Function that draws the Tail of the snake.Snake in the canvas using a Sprite.
 * @receiver where it will be drawn.
 * @param snake object with movement in the game.
 * @param pngFile input file that has the drawing of the snake.
 */
...
private fun Canvas.drawMenu() {
    ...
    drawImage("logo.png", LOGO_X_ORIGIN, LOGO_Y_ORIGIN, LOGO_SIDE, LOGO_SIDE)
}
...
/**
 * Function that draws a hacked apple from the pngFile.
 * @receiver where it will be drawn.
 * @param position to be drawn if possible.
 * @param pngFile input file that has the drawing of the apple.
 */
...
private fun Canvas.drawStatus(game: Game) {
    ...
    if (game.status == Status.PAUSE) when{
        (game.snake.body.size >= Level_WIN && snakeLocked) -> drawText(width - FIVE_CELLS, height - TEXT_BASE,
            "You Win", YELLOW)
        (game.snake.body.size <= Level_WIN && snakeLocked) -> drawText(width - FIVE_CELLS, height - TEXT_BASE,
            "You Lose", YELLOW)
