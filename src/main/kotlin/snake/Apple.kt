...
/**
 * Randomly chooses an initial apple position for any level
...
/**
 * Checks if exists a Hacked apple, and if it is possible to draw it
...
/**
 * Checks if exists an apple and if it is possible to draw it
 * @receiver Game positions
...
/**
 * Verifies if an apple gets eaten and all stats related to that
...
/**
 * Verifies if a hacked apple gets eaten and all stats related to that
 * @receiver Game proprieties.
...
fun Game.hackedAppleGetsEaten(sound: String, hackedApple: Position?, level: Int) =
	if (snake.body[0] == hackedApple) {
		...
		when (level) {
			2 -> this.copy(
				snake = this.snake.copy(toGrow = snake.toGrow + 10), score = score + 2,
				hacking = hacking.copy(hackedApple = null)
			)

			3 -> if (snake.body.size > 5)
				this.copy(
					snake = this.snake.copy(body = snake.body.dropLast(3)), score = score - 2,
					hacking = hacking.copy(hackedApple = null)
				)
